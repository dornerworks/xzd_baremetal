...
**Direct Passthrough** - This gives a guest direct and usually exclusive access to a device. Information on device passthrough can be found in the XZD User Manual. The examples provided in this guide includes passthough of UART1 to the guest(s). Note in this case since UART1 does not require interrupts or SMMU for proper use, it can be passed through to multiple guests. However, care must be taken to coordinate access of that device between the guests using it, or unspecified behavior could result.

**Paravirtualized Devices** - Dom0 provides a front-end for some devices. To talk to these, a front-end needs to be implemented in the bare metal. This requires support for the XenStore, event channels, shared memory, and all of the related Hypercalls, which are currently not implemented in the XZD Bare Metal container.

### Building the Guest Image
To build the bare metal image, run the following command targeting ELF file you want to be your payload application:

```
$ ./build_it $PATH_TO_PAYLOAD_APP/$PAYLOAD_APP.elf
```

Example: Using XZD Bare Metal with a simple standalone application built using XSDK:

1. Create a `Hello World` application in XSDK targeting the ZCU102 platform called $PROJECT\_NAME in the $XSDK\_WORKSPACE. This should create three projects, the application project call $PROJECT, a BSP project call $PROJECT_bsp, and a hardware platform project called `ZynqMP_ZCU102_hw_platform` .
2. Download the files from https://github.com/dornerworks/baremetal_XSDK, overwriting the local versions created by XSDK:
	1. ```git clone https://github.com/dornerworks/baremetal_XSDK $XSDK_REPO```
	2. ```cp $XSDK_REPO/app/src/* $XSDK_WORKSPACE/$PROJECT_NAME/src/```
	3. ```cp -r $XSDK_REPO/bsp/* $XSDK_WORKSPACE/$PROJECT_NAME_bsp/```
3. Clean and rebuild the XSDK project.
4. Download the bare metal project and run `build_it` on the ELF file. This example assumes that the `Debug` configuration was used in the XSDK project:
...
Update Dom0 booatargs:
			...
			#address-cells = <0x2>;
	  		#size-cells = <0x1>;
	  		xen,xen-bootargs = "console=dtuart dtuart=serial0 dom0_mem=512M bootscrub=0 dom0_vcpus_pin maxcpus=4 timer_slop=0";
...
Examples of these configuration files can be found at [https://github.com/dornerworks/baremetal_XSDK/tree/master/config](https://github.com/dornerworks/baremetal_XSDK/tree/master/config), or $XSDK_REPO/config if the repo was previously cloned.
